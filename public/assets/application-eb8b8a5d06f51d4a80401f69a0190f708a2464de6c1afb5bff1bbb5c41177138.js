...
(function() {
  var buildChart, getDay, getHour, getSunday, requestAltData, stringifyKey, updateChart;

  stringifyKey = function(val, reverse) {
    var locator;
    if (reverse == null) {
      reverse = false;
    }
    ...
    return (val.split(locator[reverse]).map(function(word) {
      return word[0][reverse ? 'toLowerCase' : 'toUpperCase']() + word.slice(1).toLowerCase();
    })).join(locator[!reverse]);
  };
  ...
  getDay = function(d) {
    return new Date(new Date(d.setDate(d.getDate() - 2)).setHours(0, 0, 0, 0));
  };
  ...
  buildChart = function(id, title, buttons) {
    return $("#" + id).highcharts('StockChart', {
      ...
      title: {
        text: "Calls Offered & Handled (" + title + ")"
      },
      exporting: {
        enabled: false
      },
      ...
      tooltip: {
        formatter: function() {
          var date, j, len, point, ref, s;
          date = Highcharts.dateFormat("%A, %b %e, %Y" + (id === 'min' ? ', %H:%M' : ''), this.x);
          s = "<b>" + date + "</b>";
          if (this.points != null) {
            ref = this.points;
  ...
  updateChart = function(index, data) {
    var ary, i, j, l, len, len1, length, num, point, ref, results, reverse_key, series;
    ref = Highcharts.charts[index].series;
    results = [];
    for (j = 0, len = ref.length; j < len; j++) {
      series = ref[j];
      reverse_key = stringifyKey(series.name, true);
      if (reverse_key === 'navigator') {
        reverse_key = 'calls_offered';
      }
      length = series.data.length;
      num = length - 6;
      ary = data[reverse_key].reverse();
      while (num !== length) {
        for (i = l = 0, len1 = ary.length; l < len1; i = ++l) {
          point = ary[i];
          if (point[0] === series.data[num].x) {
            console.log("Updating point at " + point[0] + " on " + series.name + " to " + point[1]);
            series.data[num].update(point[1]);
          }
        }
        num += 1;
      }
      results.push((function() {
        var len2, m, results1;
        results1 = [];
        for (i = m = 0, len2 = ary.length; m < len2; i = ++m) {
          point = ary[i];
          if (point[0] > series.data[length - 1].x) {
            console.log("Adding point at " + point[0] + " to " + series.name + " to " + point[1]);
            results1.push(series.addPoint(point, true));
          } else {
  ...
  requestAltData = function() {
    var date, url;
    date = new Date();
    url = window.location.origin;
    $.getJSON(url + ("?min_date=" + day_req + "&callback=day"), function(data) {
      console.log('Updating daily interval chart');
      updateChart(1, data);
      return window.day_req = getDay(date);
    });
    $.getJSON(url + ("?min_date=" + week_req + "&callback=week"), function(data) {
      console.log("Updating weekly interval chart");
      updateChart(2, data);
      return window.week_req = getSunday(date);
    });
    return setTimeout(requestAltData, 3600000);
  };
  ...
  $(function() {
    ...
    return (requestData = function() {
      ...
      date = new Date();
      url = window.location.origin;
      if (typeof min_req !== "undefined" && min_req !== null) {
        console.log("Updating min interval chart");
        $.getJSON(url + ("?min_date=" + min_req), function(data) {
          return updateChart(0, data);
        });
      } else {
        console.log("Running initial request");
        $.getJSON(url + "?callback=all", function(data) {
          var chart, i, j, k, len, ref, results, v;
          ...
          for (i = j = 0, len = ref.length; j < len; i = ++j) {
            ...
            results.push((function() {
              ...
              for (k in ref1) {
                v = ref1[k];
                console.log("Adding Series " + k + " to chart(" + i + ").");
                results1.push(chart.addSeries({
                  name: stringifyKey(k),
                  data: v
                }, true));
              }
        ...
        window.day_req = getDay(date);
        window.week_req = getSunday(date);
        setTimeout(requestAltData, 60000);
      }
      window.min_req = getHour(date);
      return setTimeout(requestData, 600000);
    })();
  });
